...
export const EmployeeDashboard = ({ currentUser, onSignOut }) => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      try {
        const histRes = await api.getEmployeeAttendance(employeeId, { limit: 10 });
      ...
      try {
        ...
        if (contractRes.success && contractRes.data?.length > 0) {
          // Pick active or most recent contract
      ...
      // 5. Fetch Payslips
      try {
        const payslipRes = await api.getPayslips({ employeeId });
  ...
  const handleLeaveSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Refresh requests and allocations
  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      {/* Two Column Section: Payslips & Recent Attendance History */}
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <h3 style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>My Payslips</h3>
            </div>
            <span style={{ fontSize: 12, color: "#64748b" }}>{payslips.length} record(s)</span>
          </div>
          ...
          {payslips.length > 0 ? (
            ...
            <div style={{ textAlign: "center", padding: "32px 16px", color: "#94a3b8", fontSize: 13 }}>
              No payslips issued yet. Once the payroll department executes a payrun, your payslip and PDF will appear here.
            </div>
          )}
        </div>
            ...
            <div className="modal-header" style={{ padding: "18px 24px", borderBottom: "1px solid #e2e8f0" }}>
              <div>
                <h3 style={{ fontSize: 17, fontWeight: 700, margin: 0 }}>
                  Payslip Breakdown: {selectedPayslip.payslipNumber || "N/A"}
                </h3>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
                <div style={{ background: "#f8fafc", padding: 12, borderRadius: 8, textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 17, fontWeight: 800, color: "#059669", marginTop: 2 }}>
                    ${(selectedPayslip.totalGross || 0).toFixed(2)}
                  </div>
                </div>
                <div style={{ background: "#f8fafc", padding: 12, borderRadius: 8, textAlign: "center" }}>
                ...
                <div style={{ background: "#eff6ff", padding: 12, borderRadius: 8, textAlign: "center" }}>
                  <div style={{ fontSize: 11.5, color: "#1d4ed8" }}>Net Take-Home</div>
                  ...
                  <tbody>
                    {(selectedPayslip.lines || []).map((line, idx) => (
                      <tr key={idx} style={{ borderTop: "1px solid #f1f5f9" }}>
                        <td style={{ padding: "8px 12px", fontWeight: 600 }}>{line.ruleName} ({line.code})</td>
